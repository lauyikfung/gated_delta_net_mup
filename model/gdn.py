...
class GPTConfig(PretrainedConfig):
    ...
    mup: bool = False
    mymup: bool = False
    # Reference width for muP scaling; logits_scale = hidden_size_base / hidden_size.
    # Defaults to hidden_size so that logits_scale=1 when mup=False or hidden_size_base is not set.
    hidden_size_base: int = 768  # overridden at runtime; default matches hidden_size default
...
class GPT(PreTrainedModel):
    ...
    def __init__(self, config: GPTConfig):
        ...
        self.mup = config.mup
        self.config = config
        self.post_init()  # always call: applies _init_weights (SP init) + muP overwrite if mup=True
        if config.tie_word_embeddings:
            self.tie_weights()

    def post_init(self):
        # HuggingFace does not auto-call post_init; __init__ calls it explicitly.
        # This triggers apply(_init_weights) which handles all parameter initialization,
        # including the mup-vs-SP branching inside _init_weights itself.
        super().post_init()
    ...
    def _init_weights(self, module: nn.Module):
        ...
        if isinstance(module, GatedDeltaNet) and next(module.parameters()).device.type != 'meta':
            ...
            # o_proj use hidden_std / sqrt(2 × n_layers) std
            if "o_proj" in module._get_name().lower():
                module_std = hidden_std# / math.sqrt(2 * self.config.num_hidden_layers)
