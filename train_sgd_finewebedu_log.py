...
mup_midfix = "-mymup" if mymup else mup_midfix
if init_from != "resume":
...
embedding_lr_mult = embedding_lr_multiplier * (model_hidden_size / hidden_size_base) if mup else embedding_lr_multiplier
hidden_lr_mult = 1.0

_is_gdn_model = "gdn" in model_type
if _is_gdn_model:
    from utils.gdn_sgd_utils import get_gdn_sgd_param_groups
    _gdn_gate_lr_mult = float(extra_config.get("gdn_gate_lr_mult", 1.0))
    _gdn_scalar_lr_mult = float(extra_config.get("gdn_scalar_lr_mult", 1.0))
    if mup:
        _gdn_gate_lr_mult *= math.sqrt(float(hidden_size_base) / model_hidden_size)
        _gdn_scalar_lr_mult *= math.sqrt(model_hidden_size / float(hidden_size_base))
    param_groups = get_gdn_sgd_param_groups(
        model,
        weight_decay,
        hidden_lr_mult=hidden_lr_mult,
        embedding_lr_mult=embedding_lr_mult,
        gate_lr_mult=_gdn_gate_lr_mult,
        scalar_lr_mult=_gdn_scalar_lr_mult,
    )
    if master_process:
        print(
            f"[GDN-SGD] Using fine-grained param groups: "
            f"gate_lr_mult={_gdn_gate_lr_mult:.4g}x, scalar_lr_mult={_gdn_scalar_lr_mult:.4g}x "
            f"(on top of hidden_lr_mult={hidden_lr_mult:.2f})"
        )
else:
...
# Whether this run uses a GDN model (gdn or gdn-mup) — used for muP analysis logging.
...
def _gdn_group_name(param_name: str) -> str:
    ...
    Groups:
      main_proj  — q/k/v/o/g projections (large output dim, bulk of parameters)
      gate_proj  — a_proj, b_proj (output dim = num_v_heads, small)
      scalar     — A_log, dt_bias (per-head scalars, dimension = num_v_heads)
      embedding  — token embedding / tied LM head
      conv       — short-conv weights
      mlp        — MLP intermediate/gate/down projections
...
def _log_gdn_group_stats(
    ...
    These metrics are the key evidence for a muP paper:
      - gdn/weight_norm/<group>   : ||θ_g||   — weight scale per group
      - gdn/grad_norm/<group>     : ||∇θ_g||  — raw gradient scale per group
      - gdn/effective_lr/<group>  : lr * lr_mult_g  — actual step size
      - gdn/update_norm/<group>   : lr * lr_mult_g * ||∇θ_g||  — update magnitude
      - gdn/scalar/A_log_*        : mean/std/grad_norm of A_log (recurrent eigenvalue)
      - gdn/scalar/dt_bias_*      : mean/std/grad_norm of dt_bias (recurrent timescale)
    """
    ...
    # Build param_id -> lr_mult from optimizer param groups
    ...
    for name, param in raw_model.named_parameters():
        ...
        # Log tiny GDN scalar parameters directly — their values are directly interpretable
        leaf = name.split('.')[-1]
        if leaf == 'A_log':
            v = param.data.float()
    ...
    for g in sorted(set(w_sq) | set(g_sq)):
        ...
        if g_sq.get(g, 0.0) > 0:
            ...
            # Fractional update: ‖ΔW‖/‖W‖ — should be small (~1-5%) and width-independent.
            w_norm = w_sq[g] ** 0.5
            if w_norm > 0:
...
def _log_gdn_dynamics(iter_num: int) -> None:
    """
    Capture and log intermediate activation + gradient statistics for muP/mymup verification.

    Uses temporary forward hooks + tensor.register_hook on a single eval batch
    (forward + backward with gradients). Called every eval_interval steps — not
    every training step — so overhead is negligible.

    Per sampled layer (first / middle / last GDN layer) logs:
      gdn/dyn/L{i}/q_rms          RMS per element of q (after short-conv+silu, shape [B,T,key_dim])
      gdn/dyn/L{i}/k_rms          RMS per element of k
      gdn/dyn/L{i}/qk_cos_mean    Mean cosine similarity of (q,k) per head — positive=same dir
      gdn/dyn/L{i}/qk_cos_abs     Mean |cosine| per head — 1 = q,k aligned (erasure-dominated)
      gdn/dyn/L{i}/v_rms          RMS per element of v
      gdn/dyn/L{i}/beta_mean      Mean β = sigmoid(b_proj(x)) — write strength [0,1]
      gdn/dyn/L{i}/beta_std       Std of β
      gdn/dyn/L{i}/beta_gt_half   Fraction of tokens with β>0.5 (active writes)
      gdn/dyn/L{i}/o_rms          RMS of o_proj input (output of o_norm+gate) = "o_t"
      gdn/dyn/L{i}/o_proj_out_rms RMS of o_proj output (residual contribution)
      gdn/dyn/L{i}/hidden_rms     RMS of residual stream entering this block
      gdn/dyn/L{i}/grad_q_rms     RMS of ∂L/∂q (at q_conv1d output)
      gdn/dyn/L{i}/grad_k_rms     RMS of ∂L/∂k
      gdn/dyn/L{i}/grad_o_rms     RMS of ∂L/∂o (entering o_proj) — theory: Θ(1/√d) mymup
      gdn/dyn/L{i}/grad_hidden_rms RMS of ∂L/∂h at block input — should be Θ(1) for muP

    Residual stream norms (all layers):
      gdn/dyn/residual/L{i}       RMS of residual stream after block i

    Cross-layer averages in gdn/dyn/avg/*.

    muP scaling probes in gdn/dyn/mup/* — these should be width-independent constants
    if the corresponding theory holds.  Run at multiple d values to verify:
      mup/q_hat_per_elem  = rms(q)/√head_k_dim  → Θ(1/√d) if theory holds
      mup/q_hat_x_sqrtd   = above × √d          → const if theory holds
      mup/o_rms_div_sqrtd                        → const if o_t ~ Θ(√d)  (raw, pre-norm)
      mup/grad_o_x_sqrtd                         → const if ∂L/∂o ~ Θ(1/√d)  [mymup]
      mup/grad_o_div_sqrtd                       → const if ∂L/∂o ~ Θ(√d)   [standard muP]
    """
    ...
    gdn_layers: list[tuple[int, object]] = [
        ...
        if isinstance(block.attn, GatedDeltaNet)
    ]
    if not gdn_layers:
        return
    ...
    # ── Set up hook storage ──────────────────────────────────────────────────
    captures: dict = defaultdict(dict)   # int blk_idx -> {key: tensor}
    res_caps: dict[int, torch.Tensor] = {}  # blk_idx -> residual stream after block
    hooks: list = []

    def _cap_out(blk_idx: int, key: str, *, with_grad: bool = False):
        """Forward hook: store module OUTPUT (first element if tuple)."""
        def h(m, inp, out):
    ...
    def _cap_in(blk_idx: int, key: str, *, with_grad: bool = False):
        """Forward hook: store module INPUT[0]."""
        def h(m, inp, out):
    ...
    for blk_idx, gdn in sampled_gdn:
        if gdn.use_short_conv:
            # q, k after short-conv+silu: shape [B, T, key_dim]
    ...
    # Residual stream after every block (cheap: just one .detach())
    for i, block in enumerate(raw_model.transformer.h):
    ...
    # ── One eval batch: forward + backward ───────────────────────────────────
    ...
    for blk_idx, gdn in sampled_gdn:
        ...
        if 'q' in cap and 'k' in cap:
            ...
            cos = (_F.normalize(q_h, dim=-1, eps=1e-8) *
                   _F.normalize(k_h, dim=-1, eps=1e-8)).sum(-1)  # [BT, H]
    ...
    # ── qk cosine similarity: ALL layers, with per-t position breakdown ─────────
    # gdn/dyn/qk_cos/L{i}             — avg over all (B, T, H), one line per layer in wandb
    # gdn/dyn/qk_cos_by_t/L{i}/t{t}  — avg over (B, H) at specific t, shows alignment
    #                                    pattern along the sequence
    ...
    for blk_idx, gdn in gdn_layers:
        ...
        cos_all = (_F.normalize(q_h, dim=-1, eps=1e-8) *
                   _F.normalize(k_h, dim=-1, eps=1e-8)).sum(-1)   # [B, T, H]
        # Overall mean
        log_dict[f'gdn/dyn/qk_cos/L{blk_idx}'] = float(cos_all.mean().item())
        # Per-position mean over batch and heads → [T]
    ...
    # Residual stream norms across all layers
    ...
    # ── muP scaling probes ────────────────────────────────────────────────────
    # Compare these across model widths: a constant value confirms the Θ() prediction.
    d     = float(raw_model.config.hidden_size)
    sqrtd = math.sqrt(d)
    hkd   = float(gdn_layers[0][1].head_k_dim)  # head_k_dim ∝ d in this config

    def _a(key: str) -> float | None:
        vals = agg.get(key)
        return (sum(vals) / len(vals)) if vals else None

    # q̂ = q / ‖q‖ is L2-normalised inside the kernel.
    # Per-element norm of q̂ = rms(q) / √head_k_dim.
    # Theory: q̂ ~ Θ(1/√d)  ↔  rms(q)/√head_k_dim × √d = const
    avg_q = _a('q_rms');  avg_k = _a('k_rms')
    avg_o = _a('o_rms');  avg_go = _a('grad_o_rms')

    if avg_q is not None:
        q_hat = avg_q / math.sqrt(hkd)
        log_dict['gdn/dyn/mup/q_hat_per_elem'] = q_hat          # ~ Θ(1/√d)
        log_dict['gdn/dyn/mup/q_hat_x_sqrtd']  = q_hat * sqrtd  # const if Θ(1/√d)
    ...
    # o_t = input to o_proj (after o_norm+gate).
    # Before o_norm: delta-rule output ~ Θ(√d) (q̂ inner-products over head_k_dim ∝ d keys).
    # After o_norm:  RMSNorm re-scales, expect ~ Θ(1) or Θ(1/d) depending on mymup multiplier.
    if avg_o is not None:
        log_dict['gdn/dyn/mup/o_rms']           = avg_o           # raw value
        log_dict['gdn/dyn/mup/o_rms_div_sqrtd'] = avg_o / sqrtd   # const if Θ(√d)

    # ∂L/∂o  (gradient entering o_proj, i.e. grad of o_norm output).
    # mymup theory (1/d forward multiplier on o_proj):  Θ(1/√d) → go × √d = const
    # standard muP  (no multiplier):                    Θ(√d)   → go / √d = const
    if avg_go is not None:
        log_dict['gdn/dyn/mup/grad_o_rms']       = avg_go
        log_dict['gdn/dyn/mup/grad_o_x_sqrtd']   = avg_go * sqrtd  # const if Θ(1/√d)
        log_dict['gdn/dyn/mup/grad_o_div_sqrtd']  = avg_go / sqrtd  # const if Θ(√d)
            ...
            if wandb_log:
                ...
                if _is_gdn_model:
                    if 'logit_std' in losses:
                        # muP logits scaling: std should be O(1) and width-independent
                        eval_log['gdn/eval/logit_std'] = losses['logit_std']
                    if 'a_proj_output_std' in losses:
                        # Gate health: if ≈0, a_proj is dead and the recurrent gate is input-independent
                ...
                # Runs at eval_interval frequency (same as loss eval) — not every step.
                if _is_gdn_model:
