...
if init_from != "resume":
    ...
    wandb_run_name = f"M_{model_type}_{int(param_count_m)}m_Opt_{optimizer_name}_LR_{learning_rate_base}_D_{dataset}_T_{total_tokens_B:.2f}B_time_{current_date}_jobid_{job_id}"
    # Update output directory
    out_dir = f"output/out_{model_type}_{int(param_count_m)}m_Opt_{optimizer_name}_LR_{learning_rate_base}_D_{dataset}_T_{total_tokens_B:.2f}B_time_{current_date}_jobid_{job_id}"
else:
