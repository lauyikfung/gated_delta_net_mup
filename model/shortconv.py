"""
Short, depthwise 1D convolutions used by multiple GPT variants.

This module implements a causal depthwise Conv1d over token sequences of shape (B, T, C) by:

1) Transposing to (B, C, T)
2) Left-padding with zeros to preserve length (causal padding)
3) Applying a grouped Conv1d with groups=C
4) Transposing back to (B, T, C) and applying an optional activation

`shift_right1` (a.k.a. "use x-1..x-k window")
---------------------------------------------

Some attention variants need the short convolution over keys to exclude the current token and
instead use only past tokens, i.e. the output at time index `t` should depend on:

    x[t-1], x[t-2], ..., x[t-k]

instead of the standard causal window:

    x[t], x[t-1], ..., x[t-k+1]

Conceptually, this is equivalent to computing the causal convolution on a 1-step delayed sequence
`x_delayed[t] = x[t-1]` (with `x_delayed[0] = 0`).

Implementation detail (no explicit shift op):

Let `k` be `kernel_size`. Standard causal padding uses `pad_left = k - 1`, producing an output of
length `T`.

To get a 1-step delay while keeping the operation purely "conv-like", we:

- increase the left padding by 1: `pad_left = (k - 1) + 1`
- run the same Conv1d, which yields an output of length `T + 1`
- drop the final timestep (crop `[..., :-1]`) to return to length `T`

This makes output index `t` align with the window ending at `x[t-1]`, matching the desired
`x[t-1]..x[t-k]` receptive field without a separate `shift_right_one()` in model code.
"""
